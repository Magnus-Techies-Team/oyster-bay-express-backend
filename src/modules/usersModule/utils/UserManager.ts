...
class UserManager {
  public async createUser(userData: {
    ...
    const emailMatcher = /^[-!#$%&'*+\/0-9=?A-Z^_a-z{|}~](\.?[-!#$%&'*+\/0-9=?A-Z^_a-z`{|}~])*@[a-zA-Z0-9](-*\.?[a-zA-Z0-9])*\.[a-zA-Z](-?[a-zA-Z0-9])+$/;
    if (emailMatcher.exec(userData.email)) {
      const userCreated = await new ServiceClass().createRecord({
        tableName: "users",
  ...
  public async login(userData: {
    ...
    const user = await new ServiceClass().getRecord({
      tableName: "users",
      searchBy: "login",
